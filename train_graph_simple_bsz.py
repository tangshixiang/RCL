...
class GraphNeruralNetworkLayer(nn.Module):
    def __init__(self, in_features, out_features, bias=True, withweight=True,
                withbn=True, withselfweight=True, selfloop=True, otheraggre=True,
                activation_type='relu'):
        super(GraphNeruralNetworkLayer, self).__init__()
        assert selfloop or otheraggre, "selfloop or otheraggre must not be all False!"

        self.otheraggre = otheraggre
        ...
        if withselfweight:
            self.self_fc = nn.Linear(in_features, out_features, bias=bias)
        else:
            self.register_buffer('self_fc', None)
    ...
    def forward(self, query, bank, adj=None):
        if self.otheraggre:
            if self.fc is not None:
                support = self.fc(bank)
                if adj is not None:
        ...
        if self.selfloop:
            if self.self_fc is not None:
                self_support = self.self_fc(query)
            else:
                self_support = query
        else:
            self_support = 0

        output = support + self_support
...
class MeanShift(nn.Module):
    def __init__(self, arch, m=0.99, mem_bank_size=128000, topk=5, GCN_configs_q=None, GCN_configs_t=None):
        ...
        self.predict_t = self._build_GCN(GCN_configs_t)
    ...
    def forward(self, im_q, im_t, labels):
        ...
        bsz = len(im_q)
        adj_query = torch.eye(1, self.topk).reshape(1,1, self.topk).repeat(bsz,1,1).cuda()
        ##################################################################### # TODO:
        # nodes = torch.cat([feat_q.reshape(feat_q.shape[0], 1, feat_q.shape[1]), targets[nn_index]], dim=1)
        query = self.predict_q(feat_q, targets[nn_index], adj_query)
        query = nn.functional.normalize(query, dim=1)

        # TODO: another GCN
        adj_positive_target = torch.ones(1, self.topk).reshape(1,1, self.topk).repeat(bsz,1,1).cuda()
        positive_target = self.predict_t(current_target, targets[nn_index], adj_positive_target)
        positive_target = nn.functional.normalize(positive_target, dim=1)

        # dist_q = 2 - 2 * torch.einsum('bc,kc->bk', [query, targets])
        # nn_dist_q = 2 - 2 * torch.einsum('bc,kc->bk', [query, positive_target])
        nn_dist_q = 2 - 2 * (query * positive_target).sum(-1)
