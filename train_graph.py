...
class MeanShift(nn.Module):
    ...
    def _build_GCN(self, GCN_configs):
        ...
        for i in range(num_layers):
            GCN_lists.append(self._build_GCN_Block(**GCN_configs[i]))
        GCNs = nn.Sequential(*GCN_lists)
        return GCNs

    @torch.no_grad()
    def _build_GCN_Block(self, in_features=512, out_features=512, nbaselayer=1, aggrmethod='nores',
                    withbn=True, withloop=False, withweight=True, bias=True, activation_type='relu',
                    dropout=0.0, dense=False):
        activation_type_list = {'relu': F.relu, 'identity': lambda x: x, 'softmax': F.softmax}
        activation = activation_type_list[activation_type]
        return graph_blocks.builder.MultiLayerGCNBlock(in_features, out_features, nbaselayer, withbn=withbn,
                    withloop=withloop, activation_type=activation_type, dropout=dropout,
                    aggrmethod=aggrmethod, dense=dense, bias=bias, withweight=withweight)
    ...
    def forward(self, im_q, im_t, labels):
        ...
        # query, _ = self.predict_q((feat_q, torch.eye(len(feat_q), len(feat_q) // torch.cuda.device_count()).cuda()))
        query, _ = self.predict_q((feat_q, None))
